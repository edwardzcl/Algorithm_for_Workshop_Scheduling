...
class GA:
    ...
    def Crossover_Operation(self,CHS1, CHS2, T0, J_num):
        ...
        new_os1 = list(np.zeros(T0, dtype=int))
        for k, v in enumerate(OS_1):
            if v in Set1:
                new_os1[k] = v + 1
        for i in OS_2:
            if i not in Set1:
                Site = new_os1.index(0)
                new_os1[Site] = i + 1
        new_os1 = np.array([j - 1 for j in new_os1])
        ...
        for i in OS_1:
            if i not in Set1:
                ...
                new_os2[Site] = i + 1
        new_os2 = np.array([j - 1 for j in new_os2])
    ...
    def main(self,Processing_time,J,M_num,J_num,O_num):
        ...
        plt.xlabel('Test Num')
        plt.show()
        plt.savefig('Itertion_fitness.png')
 
if __name__=='__main__':
    #from Instance_8 import Processing_time, J, M_num, J_num, O_num
    from MK01_example import Processing_time, J, M_num, J_num, O_num
    g=GA()
